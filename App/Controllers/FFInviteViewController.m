...
- (void)viewDidLoad
{
    ...
    _autocompleteView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, 320, 213)
    ...
    [_autocompleteView registerClass:[UITableViewCell class]
...
- (void)send:(UIButton*)sender
{
    ...
    NSDictionary* params = @{
        @"invitees" : [emails componentsJoinedByString:@", "],
    ...
    if (!text.length) {
        ...
        NSPredicate *predicate = [NSPredicate predicateWithBlock:^BOOL(id evaluatedObject, NSDictionary *bindings)
        {
            ...
            for (NSString* token in [text componentsSeparatedByCharactersInSet:
                ...
                // first name match
                NSRange fnRange = [contact.firstName rangeOfString:token
                                                           options:NSCaseInsensitiveSearch];
                if (contact.firstName && fnRange.location != NSNotFound) {
                    return YES;
                }
                // last name match
                NSRange lnRange = [contact.lastName rangeOfString:token
                                                          options:NSCaseInsensitiveSearch];
                if (contact.lastName && lnRange.location != NSNotFound) {
                    return YES;
                }
                // first name match
                NSRange fnRangeP = [contact.firstNamePhonetic rangeOfString:token
                                                                    options:NSCaseInsensitiveSearch];
                if (contact.firstNamePhonetic && fnRangeP.location != NSNotFound) {
                    return YES;
                }
                // last name match
                NSRange lnRangeP = [contact.lastNamePhonetic rangeOfString:token
                                                                   options:NSCaseInsensitiveSearch];
                if (contact.lastNamePhonetic && lnRangeP.location != NSNotFound) {
                    return YES;
                }
                // email match
                NSRange emRange = [evaluatedObject[1] rangeOfString:token
                                                            options:NSCaseInsensitiveSearch];
...
- (NSInteger)tableView:(UITableView*)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    NSTextCheckingResult* result = [regex firstMatchInString:txt
                                                     options:0
...
- (void)tableView:(UITableView*)tableView didSelectRowAtIndexPath:(NSIndexPath*)indexPath
{
    if (indexPath.row < _contacts.count) {
        ...
        [_bubblePicker addItem:_contacts[indexPath.row]
                         title:title
                      animated:YES];
    } else {
        NSString* title = _bubblePicker.textViewValue;
        [_bubblePicker addItem:@[
                                   [NSNull null],
                                   [title copy]
                               ]
                         title:[title copy]
