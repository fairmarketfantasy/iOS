       ...
       loadingStyle:(FFAlertViewLoadingStyle)loadingStyle
{
    ...
    if (self) {
        if (loadingStyle == FFAlertViewLoadingStylePlain) {
            ...
            [self.buttonBox addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|[spinner]|"
                                                                                   options:0
                                                                                   metrics:nil
                                                                                     views:NSDictionaryOfVariableBindings(spinner)]];
            [self.buttonBox addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[spinner]|"
                                                                                   options:0
                                                                                   metrics:nil
                                                                                     views:NSDictionaryOfVariableBindings(spinner)]];
...
- (void)_setupVerticalButtons:(NSArray*)buttons
{
    ...
    for (UIButton* butt in buttons) {
        ...
        [self.buttonBox addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[butt]|"
                                                                               options:0
                                                                               metrics:nil
                                                                                 views:NSDictionaryOfVariableBindings(butt)]];
        NSString* key = [NSString stringWithFormat:@"button%d", i];
        [buttonMapping setObject:butt
                          forKey:key];
        [vertLayout appendFormat:@"[%@(>=38)]%@", key, ((i + 1) == buttons.count ? @"" : @"-10-")];
        i++;
    }
    [vertLayout appendString:@"|"];
    [self.buttonBox addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:vertLayout
                                                                           options:0
                                                                           metrics:nil
                                                                             views:buttonMapping]];
}
...
{
    FFCustomButton* ret = [FFStyle coloredButtonWithText:str
                                                   color:[FFStyle brightBlue]
                                             borderColor:[FFStyle white]];
